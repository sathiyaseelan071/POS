    ...
    public partial class FrmCustomerCreditDebit : Form
    {
        ...
        private void ToCalcTotalAmount()
        {
            try
            {
                ...
                {
                    object resultDebit = DtView.Compute("SUM([" + CustomerCreditDebitNoteTable.ColumnName.Amount + "])", 
                        CustomerCreditDebitNoteTable.ColumnName.TransTypeCode + " = 'D'");
                    decimal decTotalDebitAmount = resultDebit != DBNull.Value ? Math.Round(Convert.ToDecimal(resultDebit), 2) : 0.00m;

                    object resultCredit = DtView.Compute("SUM([" + CustomerCreditDebitNoteTable.ColumnName.Amount + "])",
                        CustomerCreditDebitNoteTable.ColumnName.TransTypeCode + " = 'C'");
                    decimal decTotalCreditAmount = resultCredit != DBNull.Value ? Math.Round(Convert.ToDecimal(resultCredit), 2) : 0.00m;

                    decimal decTotalPendingAmount = decTotalCreditAmount - decTotalDebitAmount;

                    this.LblTotalCreditAmt.Text = this.ToConvertDecimalFormatString(decTotalCreditAmount.ToString());
                    this.LblTotalDebitAmt.Text = this.ToConvertDecimalFormatString(decTotalDebitAmount.ToString());
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                clsFrmCustomerCreditDebitNote.TransType = (string)this.CmbTransactionType.Text;
        ...
        private void SetGridStyle()
        {
            try
            {
                ...
                {
                    if (DGVColumn.Name == CustomerCreditDebitNoteTable.ColumnName.CustomerName || DGVColumn.Name == CustomerCreditDebitNoteTable.ColumnName.Remarks)
